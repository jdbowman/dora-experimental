...
class scmd(object):
    ...
    def __init__(self, bus, addr=None):
        ...
        self.addr = addr if addr is not None else self.DEFAULT_ADDR
        self.bus = bus;
    ...
    def begin(self):
        ...
        """
        # dummy read
        self.readByte(self.SCMD_ID)
        return self.readByte(self.SCMD_ID)
    ...
    def ready(self):
        """
            ...
            :rtype: boolean
        """
        statusByte = self.readByte(self.SCMD_STATUS_1)
    ...
    def enable(self):
        ...
        """
        self.writeByte(self.SCMD_DRIVER_ENABLE, 0x01)
    ...
    def disable(self):
        """
            ...
            :return: No return value
        """
        self.writeByte(self.SCMD_DRIVER_ENABLE, 0x00)
    ...
    def set_drive(self, motorNum, direction, level):
        ...
        if motorNum < 34:
            ...
            driveValue += 128
            self.writeByte(self.SCMD_MA_DRIVE + motorNum, driveValue)
    ...
    def inversion_mode(self, motorNum, polarity):
        ...
        if motorNum < 2:
            ...
            # master
            if motorNum == 0:
                self.writeByte(self.SCMD_MOTOR_A_INVERT, polarity & 0x01)
            if motorNum == 1:
                self.writeByte(self.SCMD_MOTOR_B_INVERT, polarity & 0x01)
            ...
            data = self.readByte(regTemp) & (~(1 << motorNum) & 0xFF)
            self.writeByte(regTemp, data | ((polarity & 0x01) << motorNum))
    ...
    def bridging_mode(self, driverNum, bridged):
        ...
        if driverNum < 1:
            # master
            self.writeByte(self.SCMD_BRIDGE, bridged & 0x01)
            ...
            data = self.readByte(regTemp) & (~(1 << driverNum ) & 0xFF)
            self.writeByte(regTemp, data | ((bridged & 0x01) << driverNum))
    ...
    def get_diagnostics(self):
        ...
        myDiag.U_I2C_RD_ERR = self.readByte(self.SCMD_U_I2C_RD_ERR)
        myDiag.U_I2C_WR_ERR = self.readByte(self.SCMD_U_I2C_WR_ERR)
        myDiag.U_BUF_DUMPED = self.readByte(self.SCMD_U_BUF_DUMPED)
        myDiag.E_I2C_RD_ERR = self.readByte(self.SCMD_E_I2C_RD_ERR)
        myDiag.E_I2C_WR_ERR = self.readByte(self.SCMD_E_I2C_WR_ERR)
        myDiag.LOOP_TIME    = self.readByte(self.SCMD_LOOP_TIME)
        myDiag.SLV_POLL_CNT = self.readByte(self.SCMD_SLV_POLL_CNT)

        # Count slaves
        topAddr = self.readByte( self.SCMD_SLV_TOP_ADDR)
        ...
        myDiag.MST_E_ERR        = self.readByte(self.SCMD_MST_E_ERR)
        myDiag.MST_E_STATUS     = self.readByte(self.SCMD_MST_E_STATUS)
        myDiag.FSAFE_FAULTS     = self.readByte(self.SCMD_FSAFE_FAULTS)
        myDiag.REG_OOR_CNT      = self.readByte(self.SCMD_REG_OOR_CNT)
        myDiag.REG_RO_WRITE_CNT = self.readByte(self.SCMD_REG_RO_WRITE_CNT)
    ...
    def reset_diagnostic_counts(self):
        ...
        self.writeByte(self.SCMD_U_I2C_RD_ERR, 0)
        self.writeByte(self.SCMD_U_I2C_WR_ERR, 0)
        self.writeByte(self.SCMD_U_BUF_DUMPED, 0)
        self.writeByte(self.SCMD_E_I2C_RD_ERR, 0)
        self.writeByte(self.SCMD_E_I2C_WR_ERR, 0)
        # Clear uport time
        self.writeByte(self.SCMD_LOOP_TIME, 0)
        self.writeByte(self.SCMD_MST_E_ERR, 0)
        self.writeByte(self.SCMD_FSAFE_FAULTS, 0)
        self.writeByte(self.SCMD_REG_OOR_CNT, 0)
        self.writeByte(self.SCMD_REG_RO_WRITE_CNT, 0)
    ...
    def read_remote_register(self, address, offset):
        ...
        self.writeByte(self.SCMD_REM_ADDR, address)
        self.writeByte(self.SCMD_REM_OFFSET, offset)
        self.writeByte(self.SCMD_REM_READ, 1)

        while self.busy():
            pass

        return self.readByte(self.SCMD_REM_DATA_RD)
    ...
    def write_remote_register(self, address, offset, dataToWrite):
        ...
        self.writeByte(self.SCMD_REM_ADDR, address)
        self.writeByte(self.SCMD_REM_OFFSET, offset)
        self.writeByte(self.SCMD_REM_DATA_WR, dataToWrite)
        self.writeByte(self.SCMD_REM_WRITE, 1)
    ...
    def writeByte(self, register, value):
        with SMBus(self.bus) as bus:
            return bus.write_byte_data(self.addr, register, value);